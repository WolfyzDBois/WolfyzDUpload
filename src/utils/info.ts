...
function formatUserList(userIds: string[]) {
  return userIds.map(id => `- <@${id}>`).join('\n') || '*None*';
}

function formatServerList(interaction: ChatInputCommandInteraction, serverIds: string[]) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          ...
          value: servers.map((id: string) => `- ${id}`).join('\n') || '*None*',
        },
        {
