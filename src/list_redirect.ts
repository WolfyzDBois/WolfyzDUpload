import {
  ...
  function generateEmbed(redirects: { name: string; url: string }[], page: number, perPage: number) {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
      ...
      try {
        ...
        const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
          ...
          new ButtonBuilder().setCustomId('next').setLabel('➡️').setStyle(ButtonStyle.Primary).setDisabled(redirects.length <= perPage)
        );
