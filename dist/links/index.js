...
export const command = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await sendLogEmbed(interaction, '🔗 Redirection Created', `<@${userId}> created a redirect:\n• **From**: ${shortUrl}\n• **To**: ${targetUrl}`, 0x2ecc71);
        }
        catch (err) {
            console.error('❌ FTP redirect error:', err);
            await interaction.editReply('❌ Failed to create redirect file.');
            await sendLogEmbed(interaction, '❌ Redirection Error', `An error occurred during /links by <@${userId}>`, 0xe74c3c);
        }
        finally {
            ftp.close();
        }
