...
function generateEmbed(redirects, page) {
...
export const command = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('prev').setLabel('⬅️').setStyle(ButtonStyle.Primary).setDisabled(true), new ButtonBuilder().setCustomId('next').setLabel('➡️').setStyle(ButtonStyle.Primary).setDisabled(redirects.length <= PER_PAGE));
            ...
            await sendLog(interaction, '❌ Redirection Listing Failed', `Error during /links_list by <@${userId}>`, 0xe74c3c);
        }
        finally {
            ftp.close();
        }
